...
export default class Afip {
  ...
  constructor(options = {}) {
    ...
    this.ElectronicExportBilling = new ElectronicExportBilling(this);
  }
   ...
   **/
  GetServiceTA = async function (service, firstTry = true) {
    // Declare token authorization file path
    const taFilePath = path.resolve(
      this.TA_FOLDER,
      `TA-${this.options["CUIT"]}-${service}${
   ...
   * @param service Service for token authorization
   **/
  CreateServiceTA = async function (service) {
    const date = new Date();
   ...
   **/
  WebService = function (service, options) {
